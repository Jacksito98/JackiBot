...
//-- library
const simplse = require('./whatsapp/connecting');
...
▢ *YouTube*
• https://youtube.com/channel/UCTfpv-KmckjC1rwPKqw_t1w
`
m.reply(gps)
break 
...
case 'creador': 
case 'developer':
number = '18295176783@s.whatsapp.net'
    capt = `▢ ${msg.num} : @${number.split('@')[0]}\n\n`
    capt += '▢ Instagram : https://instagram.com/jack_paymon?utm_medium=copy_link'
    await Fg.fakeLink(from, capt, thumbfg, `${msg.click}`, 'https://www.instagram.com/Jack_paymon', mek)
   /* Fg.sendContact(from, '18295176783', 'owner', {
	 key: {
          fromMe: false,
...
*≡ OWNER*
▢ Instagram : https://instagram.com/jack_paymon?utm_medium=copy_link_
▢ WhatsApp : wa.me/+18295176783 

*≡ SCRIPT*
▢ Git : ${package.homepage} 
` 
Fg.send2Button(from, teks, '*_® Jacksito98*', `Donar`, `${prefix}donate`, `⌬ ${msg.gp}s`, `${prefix}grupos`)
   break
...
▢ ${msg.tamaño} : ${resv.size}`
  ...
  case 'notify':
        ...
        break;
             case 'totag': //By Felixcrack
                      if (!isGroup) return reply(mess.only.group)
                      if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
                         encmediau = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                         file = await Fg.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
                         value = args.join(" ")
                         ...
                         ini_buffer = fs.readFileSync(file)
                         Fg.sendMessage(from, ini_buffer, document, options)
                         fs.unlinkSync(file)
                     }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
                         encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                         file = await Fg.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
                       ...
                       reply(`[ ❗] Responde a una imagen/documento/gif/sticker/audio/video con ${prefix}totag`)
                     }
                     break
        case 'tagall':
    if(!isGroup) return m.reply(msg.group)
...
const ag = await Fg.getgroupMetadata.participants(from)
...
Fg.sendMessage(from, casal, text, {quoted: mek, contextInfo: {"mentionedJid": [ag]}})
break
    ...
    //admin feature by Felixcrack
				case 'kickall':
					...
					for (let mem of groupMembers) {
						teks += `*Bye pedazos de mierdas* ${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
					mentions(teks, members_id, true)
					Fg.groupRemove(from, members_id)
					break
    case 'antilink':
    case 'antilinkwha':
    if(!isGroup) return m.reply(msg.group)
...
case 'riddle':
 ...
 case 'setprefix':
   ...
   if((args[0]) == 'multi'){
    ...
    case 'actualizar':
if (!isOwner && !isBot) return m.reply(msg.owner)
gfg = `git remote set-url origin https://github.com/Jacksito98/JackiBot.git && git pull `
exec(`${gfg}`, (err, stdout) => {
...
${menu(prefix)} 
`
    Fg.send3ButtonLoc(from, thumbfg, capt, `▢ *Jᴀᴄᴋɪ  ┃ ᴮᴼᵀ*\n▢ *Total Hits* : ${isTotalcmd}\n▢ *Usuarios* : ${User.length}\n▢ *Runtime* : ${kyun(process.uptime())}\n\n${msg.foll}`, `⦙☰ Menu Vc`, `${prefix}menuvc`, '✆ Owner', `${prefix}owner`, '⏍ Info', `${prefix}info`)
    break
